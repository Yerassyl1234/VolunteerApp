...
class ManageEventViewModel(
    ...
    private val accept: AcceptApplicantUseCase,
    private val reject: RejectApplicantUseCase,
    ...
    private fun load(eventId: String) {
        viewModelScope.launch {
            ...
            applicationRepository.getApplications(eventId).collect { applications ->
                updateState {
                    copy(
    ...
    private fun acceptApplicant(applicationId: String) = viewModelScope.launch {
        ...
        when (val result = accept(applicationId)) {
            is Result.Success -> {
            ...
            is Result.Error -> {
                ...
                sendEffect(ManageEventEffect.ShowError(result.exception.toUiText()))
            }
        }
    }
    ...
    private fun rejectApplicant(applicationId: String) = viewModelScope.launch {
        updateState { copy(processingId = applicationId) }
        when (val result = reject(applicationId)) {
            is Result.Success -> {
                updateState {
                    copy(
                        processingId = null,
    ...
    private fun archiveEvent() = viewModelScope.launch {
        ...
        sendEffect(ManageEventEffect.NavigateToArchive)
    }
}